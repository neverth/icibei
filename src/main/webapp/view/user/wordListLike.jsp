...
<div id="content" class="page-content toggled">
    ...
    <table class="table" id="tableDisPlay">
        ...
        <tr v-for="(item, index) in wordListData">
            ...
            <td>
                        <span v-if="item.userWordRelation.user_word_relation_is_like != 1"
                              style="color:white; cursor:pointer" id="likeIcon" @click="userLikeRelation(item.userWordRelation.user_word_relation_id,
                            item.userWordRelation.user_word_relation_is_like)">
                            ❤
                        </span>
                <span v-else style="color:red; cursor:pointer" id="likeIcon" @click="userLikeRelation(item.userWordRelation.user_word_relation_id,
                            item.userWordRelation.user_word_relation_is_like)">❤</span>
            </td>
...
<script>
    ...
    function getUserWordList(pageNum) {
        $.ajax({
            type: "post",
            url: "${request.getContextPath()}/getLikeWordList/<%=user.getUser_id()%>/" + pageNum,
            dataType: "json",
            data: {},
            success: function (result) {
                console.log(result);
